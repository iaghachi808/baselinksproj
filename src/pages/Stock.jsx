        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mt-8 px-4 w-full max-w-6xl mx-auto">
          {filteredProducts.map((product) => (
            <Link to="/custom" key={product.id}>
              <div
                className={`rounded-lg p-8 relative overflow-hidden ${product.id % 2 === 0 ? 'bg-teal-200' : 'bg-orange-200'} text-white text-2xl font-bold text-center backdrop-blur-sm bg-opacity-90 border border-gray-200 h-48 flex items-center justify-center ${isVisible ? 'animate-fade-in-up' : ''} ${`delay-${product.id * 100}`}`}
        ...
        {(hoveredProduct || selectedProduct) && (
          <div className="fixed bottom-4 left-1/2 transform -translate-x-1/2 w-11/12 max-w-2xl bg-orange-100 bg-opacity-95 rounded-xl p-6 shadow-2xl z-20 animate-slide-up">
            ...
            <div className="flex justify-between items-center">
              <button
                onClick={(e) => { e.preventDefault(); handleProceed(selectedProduct || hoveredProduct); }}
